...
func New(conf *Config) (*DB, error) {
	...
	trips := db.Collection(tripCollectionName)
	if trips == nil {
		return nil, fmt.Errorf("db: no collection found with name \"%s\" in database", tripCollectionName)
	}

	return &DB{client, trips}, nil
}
