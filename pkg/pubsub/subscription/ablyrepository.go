...
// NewAblyRepository creates a subscription repository to manage Ably REST
// channels.
func NewAblyRepository(client *ably.RestClient) (Repository, error) {
	if client == nil {
...
// Create creates a new Ably REST channel for the given topic, and a
...
func (r *AblyRepository) Create(topic string) (Subscription, error) {
	...
	channel := r.client.Channels.Get(topic, nil)
